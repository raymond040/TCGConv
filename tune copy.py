...
if __name__ == '__main__':
    ...
    setup_seed(args)
    setup_device(args)
    args.csvPath = os.path.join(args.root, 'Tune/'+args.dataset_name+'/H_'+args.model_type+'_'+str(args.num_version)+'.csv')
    big_dct = {
            'avg_F1': [],
            'avg_AP': [],
            'alpha': [],
    ...
    for i in range (0, args.n_run):
        ...
        avgAP, avgF1, p, r = main(config)
        print(config)
        print(str(avgAP)+str(avgF1)+str(p)+str(r))
        ...
        big_dct['hid_chnl'].append(config['hidden_chnl'])
        ...
        big_df.to_csv(args.csvPath, index=False, header=True)
    # main()