    ...
    if args.mode == 'test':
        ...
        for i in range(args.num_groups - 1):
            ...
            for epoch in range(1, args.num_epochs + 1):
                ...
                train_output = test(args=args, model=best_all_dct['model'][i], graph=train_group, loss_fn=loss_fn, epoch=epoch,  mode = 'Train')
                train_loss = train_output['loss'] 
                test_output = test(args = args, model = best_all_dct['model'][i], graph = test_group, loss_fn = loss_fn, epoch = epoch, mode = 'Test')
