...
class TemporalGCNConv(MessagePassing):
    ...
    def aggregate(self, inputs: Tensor, index: Tensor, node_ts_j: Tensor,
                  ptr: Optional[Tensor] = None,
...
def TCGConv_Trainer(args,config,Train_Groups, Test_Groups):
        ...
        for group1, group2 in zip(Train_Groups, Test_Groups):
            ...
            if args.type_ED == "sub":
                if i == 0:
                    ...
                    optimizer = torch.optim.Adam(model.parameters(), lr=config['lr']) #dont have weight decay
                    loss_fn = focal_loss(alpha=config['alpha'], gamma=config['gamma'], num_classes=2, size_average=True)
                    best_all_dct = {
                        'AP':[],
                        'P':[],
            ...
            optimizer = torch.optim.Adam(best_all_dct['model'][i].parameters(), lr=config["lr"])
            ...
            if i == args.num_groups - 2:
                optimizer = torch.optim.Adam(best_all_dct['model'][-1].parameters(), lr=config["lr"])
